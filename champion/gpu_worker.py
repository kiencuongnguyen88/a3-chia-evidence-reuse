...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--n',type=int,required=True);ap.add_argument('--variant',choices=['baseline','candidate'],required=True);ap.add_argument('--samples',type=int,default=31);ap.add_argument('--output',required=True);args=ap.parse_args()
    dest=pathlib.Path(args.output);dest.parent.mkdir(parents=True,exist_ok=True)
    result={'n':args.n,'variant':args.variant,'pid':os.getpid(),'samples':[],'telemetry':[],'status':'RUNNING','batch_calls':100,'warmup_calls':30,'started_utc':datetime.datetime.now(datetime.timezone.utc).isoformat()}
    ...
    try:
        ...
        for i in range(args.samples):
            ...
            for label in row['order']:
                ...
                for _ in range(100):functions[label]()
                end.record();end.synchronize()
                row[label+'_ms']=start.elapsed_time(end)/100
                row[label+'_wall_ms']=(time.perf_counter()-wall)*10
